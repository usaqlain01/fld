...
$(function(){
	$(".audioPlayer__playPause").click(function(){
		...
		if (playing) {
			audio.pause();
			vikings.analytics.audioEvent("audio-pause", audio.currentSrc)
		}
		else {
	...
	$(".audioPlayer audio").on("play", function(){
		vikings.analytics.audioEvent("audio-play", this.currentSrc)
	});
	$(".audioPlayer audio").on("ended", function(){
		vikings.analytics.audioEvent("audio-listen-100%", this.currentSrc)
	});
	$(".audioPlayer audio").on("timeupdate", function(){
		var $this = $(this);
		if (this.currentTime > this.duration / 2 && !$this.data("half-played")) {
			$(this).data("half-played", true)
			vikings.analytics.audioEvent("audio-listen-50%", this.currentSrc);
		}
	});
});

//drawer
function trayOpen(headerButton, whichSection){
	$('body').addClass('trayOpen trayOpen--' + whichSection)
	vikings.analytics.headerNavClick(headerButton);
}

$(function(){
	$(document).on("click", ".tray a", function() {
		vikings.analytics.trayClick(this);
	});
})

function trayClose(){
	$('body').removeClass("trayOpen")
	vikings.analytics.trayClick(null, "tray-close");
	$(".tray").one('transitionend', function() { 
		$("body").removeClass("trayOpen--visit trayOpen--educators trayOpen--programs"); 
	});
}

$(function(){
	$(".tray").on('transitionend', function(event) {
...
window.vikings.analytics.sponsorLogoClick = function(which) {
	window.vikings.analytics.event("one-page", "logo-click", which)
}
...
window.vikings.analytics.trayClick = function(link, overrideText) {
	var text = overrideText || $.trim($(link).text());
	window.vikings.analytics.event("one-page", "tray-click", text)
}
...
window.vikings.analytics.audioEvent = function(eventName, clipName) {
	window.vikings.analytics.event("one-page", eventName, clipName)
}

window.vikings.analytics.event = function(category, action, label, noninteraction) {
...
var hasBeenToAnySectionBesidesIntro = false;
...
$(function(){
	...
	var changeHashOnScroll = function(down, scrollTop, last){
		...
		if (hash != newHash) {
			hash = newHash;
			hasBeenToAnySectionBesidesIntro = hasBeenToAnySectionBesidesIntro || hash != "introduction";
			if (!vikings.suspendScrollEvents && hasBeenToAnySectionBesidesIntro) 
				vikings.analytics.event("one-page", "section-scroll-enter", hash, true);
