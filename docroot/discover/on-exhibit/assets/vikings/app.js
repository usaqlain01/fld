...
$(function(){
	$(".audioPlayer__playPause").click(function(){
		...
		if (playing) {
			audio.pause();
			common.analytics.audioEvent("audio-pause", audio.currentSrc)
		}
		else {
	...
	$(".audioPlayer audio").on("play", function(){
		common.analytics.audioEvent("audio-play", this.currentSrc)
	});
	$(".audioPlayer audio").on("ended", function(){
		common.analytics.audioEvent("audio-listen-100%", this.currentSrc)
	});
	$(".audioPlayer audio").on("timeupdate", function(){
		...
		if (this.currentTime > this.duration / 2 && !$this.data("half-played")) {
			...
			common.analytics.audioEvent("audio-listen-50%", this.currentSrc);
		}
	});
});
...
function trayOpen(headerButton, whichSection){
	...
	common.analytics.headerNavClick(headerButton);
}

$(function(){
	$(document).on("click", ".tray a", function() {
		common.analytics.trayClick(this);
	});
})

function trayClose(){
	$('body').removeClass("trayOpen")
	common.analytics.trayClick(null, "tray-close");
...
$(function(){
	...
	common.onScroll(function(down, top, last) {
	...
	var changeHashOnScroll = function(down, scrollTop, last){
		...
		if (hash != newHash) {
			...
			if (!vikings.suspendScrollEvents && hasBeenToAnySectionBesidesIntro) 
				common.analytics.event("one-page", "section-scroll-enter", hash, true);
			vikings.changeHash(newHash);
		}
	}
	common.onScroll(changeHashOnScroll);
});
...
vikings.zoom.toggle = function(zoom) {
	...
	common.analytics.event("one-page", (close ? "modal-close" : "modal-open"), modalName) 
}
vikings.zoom.nav = function(zoomButton, event, back) {
	...
	common.analytics.event("one-page", (back ? "modal-back" : "modal-next"), modalName) 
	return false;
}
